...
module.exports = class bitfinex extends Exchange {
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        await this.loadMarkets();
        if (type == "stop_limit") type = 'stop limit'
        const request = {
            'symbol': this.marketId(symbol),
            'side': side,
        ...
        if (type == "stop limit") {
            request['price_trailing'] = this.priceToPrecision(symbol, params.stopPrice);
            request[`${side}_stoplimit_price`] = this.priceToPrecision(symbol, price);
        }
