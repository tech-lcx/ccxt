...
module.exports = class kucoin extends Exchange {
  ...
  async fetchOHLCV(symbol, timeframe = '15m', since = undefined, limit = undefined, params = {}) {
    ...
    const request = {
      'symbol': marketId,
      'endAt': params.endAt ? Math.floor(params.endAt / 1000) : this.seconds(), // required param
      'type': this.timeframes[timeframe],
    };
    if (since !== undefined) {
      request['startAt'] = Math.floor(since / 1000);
    }
    const response = await this.publicGetMarketCandles(this.extend(request, {}));
    const responseData = response['data'];
    return this.parseOHLCVs(responseData, market, timeframe, request.startAt * 1000, limit);
  }
