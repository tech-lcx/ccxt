...
module.exports = class huobipro extends Exchange {
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (type === 'limit' || type === 'ioc' || type === 'limit-maker' || type === 'stop-limit') {
            request['price'] = this.priceToPrecision(symbol, price);
        }
        if (type === 'stop-limit') {
            request['stop-price'] = this.priceToPrecision(symbol, params.stopPrice);
            if (side == 'sell') request['operator'] = 'gte';
            else request['operator'] = 'lte';
        }
